    ...
    return (
      <tr key={this.props.collectionParceldata.id}>
        ...
        <td className='p-3'>
          ...
          <Link
            to={`/editHubParcel/${this.props.collectionParceldata.id}`}
...
export default class HubParcelTable extends React.Component {
  render() {
    const result = this.props.data.map((c, index) => (
      <HubParcelRow key={c.id} collectionParceldata={c} index={index} />
    ));
    return (
